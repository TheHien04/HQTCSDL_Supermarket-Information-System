...
go
--------------------------------------- KHO HÀNG: ĐẶT HÀNG DƯỚI NGƯỠNG 70% ----------------------------------------
-- Query 1: tính số lượng nên đặt cho SP015 (đã seed dưới 70%)
DECLARE @SLNenDat INT;
EXEC usp_TinhHangCanDat @IDSanPham = 'SP015', @SLNenDat = @SLNenDat OUTPUT;
SELECT @SLNenDat AS SoLuongNenDat;

-- Query 1 (tiếp): thực hiện đặt hàng
DECLARE @TrangThai BIT;
EXEC usp_ThucHienDatHang @IDSanPham = 'SP015', @SLDat = NULL, @TrangThaiDatHang = @TrangThai OUTPUT;
SELECT @TrangThai AS DatHangThanhCong;
SELECT * FROM KhoHang WHERE IDSanPham = 'SP015';
SELECT TOP 1 pd.*, hd.*
FROM PhieuDat pd
JOIN HangDat hd ON pd.MaPhieuDat = hd.MaPhieuDat
WHERE hd.IDSanPham = 'SP015'
ORDER BY pd.NgayDat DESC, pd.MaPhieuDat DESC;

--------------------------------------- KHO HÀNG: LOST UPDATE KHI NHẬP KHO ĐỒNG THỜI ----------------------------------------
-- Tạo bản unsafe có delay
CREATE OR ALTER PROCEDURE usp_CapNhatSL_HCG_1
    @IDSanPham VARCHAR(15),
    @SL_HangGiaoThem INT
AS
BEGIN
...
-- Chạy query 1
EXEC usp_CapNhatSL_HCG_1 @IDSanPham = 'SP001', @SL_HangGiaoThem = 5;

-- Chạy query 2 song song
EXEC usp_CapNhatSL_HCG @IDSanPham = 'SP001', @SL_HangGiaoThem = 3;

-- Quan sát: bản unsafe dễ lost update trên SL_HT/SL_CG;
-- bản production dùng SERIALIZABLE + XLOCK nên giữ nhất quán.

--------------------------------------- KHO HÀNG: ĐẶT HÀNG CUỐI NGÀY ----------------------------------------
EXEC usp_DatHangCuoiNgay;
GO
